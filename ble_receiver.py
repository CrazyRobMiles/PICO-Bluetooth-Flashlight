...
from PixieLight import PixieLight
...
class BLE_receiver:
    ...
    def update(self):
        ...
        while self.is_connected():
            self.read(callback=self.set_light)
            self.output.update()
            time.sleep_ms(200)
    ...
    def set_light(self,result):
        self.output.unpack(result)

def run():
    print("Light Receiver Starting")
    pixieLight = PixieLight(0.2)
    receiver = BLE_receiver(pixieLight)
    while(True):
        receiver.update()
        time.sleep_ms(50)
